...
class GetCampaignByIdTest {
    ...
    @Test
    fun getCampaign_success_no_retry() {
        ...
        try {
            ...
            val campaign = client.getCampaignById(100L)
    ...
    @Test
    fun getCampaign_retries_on_429_then_success() {
        val server = MockWebServer()
        server.enqueue(
            MockResponse()
                .setResponseCode(429)
    ...
    @Test
    fun getCampaign_fails_after_max_retries_rate_limit() {
        val server = MockWebServer()
        repeat(4) {
            server.enqueue(
                MockResponse()
        ...
        try {
            val client = SimplifiApiClient(server.url("/").toString(), "app", "user", 3, 1, 1, 3, 1, 1)
            val ex = assertThrows(SimplifiRateLimitRetriesExhausted::class.java) { client.getCampaignById(100L) }
    ...
    @Test
    fun getCampaign_retries_on_500_then_success() {
        val server = MockWebServer()
        server.enqueue(
            MockResponse()
                .setResponseCode(500)
    ...
    @Test
    fun getCampaign_fails_after_max_retries_server_error() {
        val server = MockWebServer()
        ...
        try {
            val client = SimplifiApiClient(server.url("/").toString(), "app", "user", 3, 1, 1, 3, 1, 1)
            val ex = assertThrows(SimplifiServerExceptionRetriesExhausted::class.java) { client.getCampaignById(100L) }
